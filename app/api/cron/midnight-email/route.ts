...
export async function GET(request: NextRequest) {
  ...
  const habitResults = todayHabits.map((h) => {
    const isSkipped = skipDay && h.skippable;
    const completed = completedKeys.has(h.key);
    return {
      label: h.label,
  ...
  const completedTasksHtml = completedTasks.length > 0
    ? completedTasks.map((t) => `<li style="color:#22c55e;">✅ ${t.title}</li>`).join("")
    : '<li style="color:#9ca3af;">No tasks completed</li>';

  const incompleteTasksHtml = incompleteTasks.length > 0
    ? incompleteTasks.map((t) => `<li style="color:#ef4444;">❌ ${t.title}</li>`).join("")
    : "";
